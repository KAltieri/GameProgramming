...
int main(int argc, char *argv[])
{
    ...
    Entity ship(glm::vec3(0.0f, -0.85f, 0.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.2f, 0.5f, 0.0f), glm::vec3(10.0f, 1.0f, 0.0f), 0.0f, temp);
    test.ship = ship;
    ...
    while (!done) {
        ...
        switch(gameMode)
        {
            case 0:
                ...
                {
                    gameMode = 1;
                    for(Entity& enable : enemy1)
                    {
                        enable.isEnabled = true;
                    }
                    for(Entity& enable : enemy2)
                    {
                        enable.isEnabled = true;
                    }
                    for(Entity& enable : enemy3)
                    {
                        enable.isEnabled = true;
                    }
                }
...
//                ship.Draw(program);
                ...
                for(Entity& bullet : bullets)
                {
                    ...
                    for(Entity& update : enemy1)
                    {
                        ...
                        if(update.position.y == ship.position.y && update.isEnabled)
                        {
                            gameMode = 0;
                            break;
                        }
